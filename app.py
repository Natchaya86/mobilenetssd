...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if msg == "สวัสดีค่ะ":
           ...
           headers = request.headers json_headers = ({k:v for k, v in headers.items()}) json_headers.update({'Host':'bots.dialogflow.com'}) url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/8b461414-8e96-45d6-bf83-dc30e8b4d15e" requests.post(url,data=json_line, headers=json_headers)
    elif msgType == "image":
        try:
